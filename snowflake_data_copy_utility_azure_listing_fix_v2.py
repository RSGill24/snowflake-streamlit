...
# external stage. The stage must exist in Snowflake for SELECT/COPY access.
CUSTOMER_AZURE_STORAGE_INTEGRATION = "AZURE_STGEHAEDP_STR_INT"
# The actual stage is the source of truth for the Azure location. The customer
# stage currently resolves to the DEV storage account shown by DESC STAGE.
...
# Supported Azure Blob source file types.
AZURE_SUPPORTED_FILE_TYPES = ("PARQUET", "CSV", "TXT")
...
st.write("Streamlit execution context")
st.write(ctx)
...
def preview_stage_parquet_file(stage_name, file_name, limit=10):
    """Preview a supported Azure file directly from the external stage."""
    source_ref = stage_file_reference(stage_name, file_name)
    query = f"""
        SELECT *
        FROM {source_ref}
        {azure_file_format_for_select(file_name)}
        LIMIT {int(limit)}
    """
...
def copy_stage_parquet_to_table(
    ...
    """Load a supported Azure file into an existing Snowflake table."""
    ...
    match_by_name = "MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE" if file_type == "PARQUET" else ""
    query = f"""
        COPY INTO {target}
        FROM {source}
        {azure_file_format_for_copy(file_name)}
        {match_by_name}
...
def get_azure_file_type(file_name):
    """Return supported Azure file type from selected file name."""
...
def azure_file_format_for_select(file_name):
    file_type = get_azure_file_type(file_name)
    if file_type == "PARQUET":
        return "(FILE_FORMAT => (TYPE => PARQUET))"
    return "(FILE_FORMAT => (TYPE => CSV, FIELD_OPTIONALLY_ENCLOSED_BY => '\"'))"


def azure_file_format_for_copy(file_name):
    file_type = get_azure_file_type(file_name)
    if file_type == "PARQUET":
        return "FILE_FORMAT = (TYPE = PARQUET)"
    return "FILE_FORMAT = (TYPE = CSV, FIELD_OPTIONALLY_ENCLOSED_BY = '\"')"
...
with source_col:
    ...
    if source_type == "Snowflake Table":
        ...
        # Always use the fully qualified customer stage; do not reuse legacy session state.
        azure_stage = DEFAULT_AZURE_STAGE
        st.text_input(
        ...
        # Do not run LIST @stage during normal page rendering.
        # LIST can require stage/database privileges that are not needed for
        # direct SELECT/COPY access to a known staged file. The previous
        # implementation caused the Azure source screen to fail with:
        # "Database 'BRONZE_DB' does not exist or not authorized".
        #
        # The user enters the path relative to the configured external stage.
        # Preview/COPY then use the same fully qualified stage reference.
        azure_file_name = st.text_input(
            "Azure Blob File Path",
            value="",
            key="azure_file_name_manual_v4",
            placeholder="path/file.parquet, path/file.csv or path/file.txt",
            help=(
                "Enter the path relative to the configured Snowflake external "
                "stage, for example churn_model/fehb_churn_label.parquet."
            ),
        ).strip()
        ...
        if azure_file_name:
            try:
                ...
                st.caption(f"Detected file type: `{azure_file_type}`")
...
with edit_tab:
    if source_type == "Azure Blob Storage":
        ...
        st.caption(
            "Snowflake reads the Parquet file directly from the external stage and "
            "loads it into the selected target table. No file-format object is required."
        )
        ...
        if not target_schema_selected:
            ...
            azure_target_tables = (
                ...
                if schema_exists(target_db, target_schema)
                else []
            )
            if not azure_target_tables:
                st.error(
                    "No target tables were found. Create the target table in Snowflake first, "
                ...
                if azure_copy_mode == "REPLACE":
                    st.warning(
                        "REPLACE truncates the selected target table before loading the Parquet file."
                    )

                st.markdown("### Azure File Preview")
                preview_col, refresh_col = st.columns(2)
                with preview_col:
                    preview_azure = st.button(
                        "🔍 Preview Azure File",
                        type="primary",
                        use_container_width=True,
                        key="preview_azure_file",
                    )
                with refresh_col:
                    refresh_files = st.button(
                        "🔄 Refresh Stage Files",
                        use_container_width=True,
                        key="refresh_azure_files",
                    )
                ...
                if preview_azure:
                    try:
                        preview_df, preview_sql = preview_stage_parquet_file(
                            azure_stage, azure_file_name, 10
                        )
                        st.markdown("**Snowflake Query Preview**")
                        st.code(preview_sql, language="sql")
                        st.dataframe(preview_df, use_container_width=True)
                    except Exception as exc:
                        st.error(f"Azure Parquet preview failed: {exc}")

                st.markdown("### Load Summary")
                st.write("**Source Type:** `Azure Blob Storage / Parquet`")
                ...
                load_preview += (
                    f"COPY INTO {target_ref}\n"
                    f"FROM {source_ref}\n"
                    f"{azure_file_format_for_copy(azure_file_name)}\n"
                    f"{'MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE' if get_azure_file_type(azure_file_name) == 'PARQUET' else ''};"
                )
                st.markdown("### Copy SQL Preview")
                st.code(load_preview, language="sql")

                confirm_azure = st.checkbox(
                    "I confirm that I want to load this Azure file into the selected target table.",
                    key="confirm_azure_copy",
                )
                if confirm_azure and st.button(
                    "🚀 LOAD AZURE FILE",
                    type="primary",
                    use_container_width=True,
                    key="execute_azure_copy",
                ):
                    run_id = str(uuid.uuid4())
                    try:
                        with st.spinner("Snowflake is loading the Azure file..."):
                            loaded_rows, executed_sql = copy_stage_parquet_to_table(
                                azure_stage,
                                azure_file_name,
                                target_db,
                                target_schema,
...
with analysis_tab:
    ...
    # Initialize analysis state on every Streamlit rerun.
    analysis_db = None
    analysis_schema = None
    analysis_tables = []

    analysis_scope_options = (["Source", "Target"] if source_type == "Snowflake Table" else (["Azure Blob Parquet", "Target"] if source_type == "Azure Blob Storage" else ["Target"]))
    analysis_source = st.radio("Analysis Scope", analysis_scope_options, horizontal=True, key="analysis_scope")
    if analysis_source == "Azure Blob Parquet":
        ...
        # Keep Azure analysis on the same fully qualified customer stage.
        # This also avoids stale Streamlit session-state values from older
        # versions of the application.
        parquet_stage = DEFAULT_AZURE_STAGE
        st.text_input(
            "External Stage",
            value=DEFAULT_AZURE_STAGE,
            key="parquet_analysis_stage_v2",
            disabled=True,
        )
        ...
        try:
            parquet_files = [
                ...
                if path.rstrip("/").lower().endswith(".parquet")
            ]
        except Exception as exc:
            parquet_files = []
