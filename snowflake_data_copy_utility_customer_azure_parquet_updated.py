...
with source_col:
    ...
    if source_type == "Snowflake Table":
        ...
        st.caption(
            ...
            "stage. All customer files are expected to be Parquet; no customer-created "
        ...
        # Use the customer-configured fully qualified stage as the single source
        # of truth. Do not reuse the legacy Streamlit session-state key from
        # previous app versions, because it can contain the old/incorrect
        # BRONZE_DB.AZURE_STGEHAEDP_STG value.
    ...
    if source_type == "Azure Blob Storage":
        ...
        if not target_schema_selected:
            ...
            if not azure_target_tables:
                ...
                st.markdown("### Azure Parquet Preview")
                preview_col, refresh_col = st.columns(2)
                with preview_col:
                    preview_azure = st.button(
                        "🔍 Preview Azure Parquet File",
                        type="primary",
                ...
                confirm_azure = st.checkbox(
                    "I confirm that I want to load this Azure Parquet file into the selected target table.",
                    key="confirm_azure_copy",
                )
                if confirm_azure and st.button(
                    "🚀 LOAD AZURE PARQUET FILE",
                    type="primary",
                    use_container_width=True,
                    ...
                    try:
                        with st.spinner("Snowflake is loading the Azure Parquet file..."):
                        ...
                        st.success("✅ Azure Parquet file loaded successfully.")
