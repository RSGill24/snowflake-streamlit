...
APP_DATABASE = "APPS_DB"
APP_SCHEMA = "EDW"
# Audit logging is intentionally disabled for this customer deployment.
AUDIT_TABLE = None
...
DEFAULT_AZURE_FILE_FORMAT = f"{APP_DATABASE}.{APP_SCHEMA}.CSV_FILE_FORMAT"
...
def write_audit(
    ...
    The function remains as a no-op so existing call sites do not need to be
    changed and the absence of DATA_COPY_AUDIT cannot cause a copy/load
    operation to fail.
    """
    return None
...
def preview_stage_file(stage_name, file_name, file_format, target_columns, limit=10):
    ...
    query = f"""
        ...
        (FILE_FORMAT => '{escape_sql_string(str(file_format).strip())}') t
        LIMIT {int(limit)}
    """
...
def copy_stage_file_to_table(stage_name, file_name, file_format,
    ...
    query = f"""
        COPY INTO {target}
        FROM {source}
        FILE_FORMAT = (FORMAT_NAME = '{escape_sql_string(str(file_format).strip())}')
...
with source_col:
    ...
    if source_type == "Snowflake Table":
        ...
        if not source_schemas:
            ...
            st.stop()
        source_schema = st.selectbox("Source Schema", source_schemas, key="source_schema")
            ...
            if not azure_target_tables:
                ...
                if confirm_azure and st.button("🚀 LOAD AZURE FILE", type="primary", use_container_width=True, key="execute_azure_copy"):
                    ...
                    try:
                        ...
                        try:
                            write_audit(
                                run_id, "AZURE_BLOB_STORAGE", azure_stage,
                                azure_file_name, target_db, target_schema,
                                azure_target_table, "AZURE_BLOB_LOAD", 0, 0,
                                "FAILED", str(exc)
                            )
                        except Exception:
                            pass
                    ...
                    if not selected_source_tables:
                        ...
                        try:
                            ...
                            for result in execution_results:
                                write_audit(run_id, source_db, source_schema, result["Source Table"], target_db, target_schema, result["Target Table"], copy_operation, result["Source Rows"], result["Target Rows"], "SUCCESS", None)
                            ...
                            # Audit logging is disabled for this deployment.
                            pass
...
with analysis_tab:
    ...
    analysis_scope_options = ["Source", "Target"] if source_type == "Snowflake Table" else ["Target"]
    analysis_source = st.radio("Analysis Scope", analysis_scope_options, horizontal=True, key="analysis_scope")
    if analysis_source == "Source":
    ...
    if not analysis_schema or not analysis_tables:
        st.info("Select a valid target schema above, or use Source scope, to analyze data.")
    else:
        analysis_mode = st.radio(
            "Analysis Type",
            ["Single Table Analysis", "Cross Table Analysis", "Fraud & Waste Analysis", "Sentiment Analysis"],
            horizontal=True,
